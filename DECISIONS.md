...
## 2026-08-03 (later still) — surrogates are not internal standards

**Correction from the lab, and it invalidates part of decision ①.** The 21
labelled compounds are two different things doing two different jobs:

* a **surrogate** (extracted internal standard) is added BEFORE extraction, so a
  dilution dilutes it along with the sample and its area falls by the factor;
* the **injection internal standard** — 13C4-PFOA here — is added at
  reconstitution, AFTER the dilution, so its area does NOT scale.

Decision ① had split the check on neat-vs-dilution. The correct axis is ROLE.
Surrogate areas are corrected by the recorded dilution factor before comparison;
the injection IS is compared as measured, so a drop in a dilution is a real
finding rather than arithmetic.

**The notation already existed everywhere except where it was used.** SENAITE
carries `pfas_role` on the AnalysisService — 1 `injection_is`, 26 `surrogate`,
47 `analyte` — `analyte_reference.INTERNAL_STANDARDS` records the role per row,
and the profile names `surrogate_is: "M4PFOA"`. The pipeline kept a flat
`_FDA_IS_DISPLAY_NAMES` list with no roles and applied one rule to all 21. Same
"recorded in one place, never carried to where it is used" shape as the dilution
relationship, the tier membership and the analyte aliases.

**And a fourth instance of the migration residue.** `reload_criteria` read the
FLAT `calibration` / `is` / `confirmation` keys, which
`migrate_profile_structure` retired and which are all null on a migrated
profile — so every threshold silently fell back to `_DEFAULT_CRITERIA`. The
hardcoded defaults happen to carry the same numbers as the nested structure
(ion ratio 30%, IS 50–150%, S/N 3), which is exactly why nobody noticed:
**editing any of these in the Method Profile UI had no effect on the engine.**
The engine now reads `instrument_verification.*` with the flat keys as legacy
fallback, and the retired keys are deleted from the stored profiles.

Worth recording: the 50–150% IS window is annotated in the profile as
"Lab SOP screen; FDA method sets no numeric IS-area limit" — it is the lab's
own screen, not a method requirement.

**What this dataset can and cannot show.** All 21 labelled compounds scale ~10×
in the dilutions of this file, 13C4-PFOA included at 8.97×, so the export was
generated by scaling everything uniformly and does not model the injection IS.
The logic is verified on it — a surrogate corrects to 70.5% of the ICAL average
